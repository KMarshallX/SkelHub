...
def point_inside_foreground(point: np.ndarray, foreground: np.ndarray) -> bool:
    """Match checker's closed-cell test for one voxel-space point."""
    cell_radius = 0.5 + 1e-9
    shape = np.asarray(foreground.shape, dtype=np.int64)
    if np.any((point < -cell_radius) | (point > shape - 1 + cell_radius)):
        return False

    lowest = np.ceil(point - cell_radius).astype(np.int64)
    highest = np.floor(point + cell_radius).astype(np.int64)
    for offset in np.ndindex(2, 2, 2):
        candidate = lowest + offset
        if np.all((candidate <= highest) & (candidate >= 0) & (candidate < shape)):
            if foreground[tuple(candidate)] != 0:
                return True
    return False
...
def check_graph_nodes(
    ...
    for vertex in graph.vs:
        ...
        inside = point_inside_foreground(voxel_pos, foreground)
...
def main(
    ...
    update(5, f"Loading foreground volume: {Path(args.input_fore).name}")
    ...
    update(25, f"Foreground labeled: {component_count} connected components (26-connectivity)")

    update(35, f"Loading primary graph: {Path(args.input_graph).name}")
    graph = load_graph(args.input_graph, label="--input-graph")
    update(None, f"Checking {graph.vcount()} graph nodes for escaping points")
    ...
    if escaping_count == 0:
        ...
        update(95, "All graph nodes are confined; no patches needed")
        return 0
    ...
    update(55, f"Preparing {len(crops)} affected component patches from {escaping_count} escaping nodes")
    foreground_outputs = []
    raw_nifti_outputs = []
    graph_outputs = []
    rasterized_graph_outputs = []
    for index, crop in enumerate(crops, start=1):
        update(55 + int(40 * (index - 1) / len(crops)), f"Writing component {crop.label} patches ({index}/{len(crops)})")
    ...
    update(95, f"Wrote {len(foreground_outputs)} foreground and {len(graph_outputs)} graph patches")
    return 0
