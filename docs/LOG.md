...
- Made graphgen `centerline_voxels`, `centerline_voxel_points`, and
  ...
  coordinates.
- Added rounded, 26-connected endpoint segments to the integer path and
  updated `num_centerline_voxels` accordingly.
...
- Extended local ignored regression coverage in
  ...
  changed.
- Modified `docs/postprocessing.md`, `docs/StructuredOutput.md`, and
  `docs/LOG.md`.
...
- Oriented every path to igraph's serialized source and target because an
  undirected edge can be normalized into the reverse of its insertion order.
- Preserved all regular-component voxels and added only the connector voxels
...
- Endpoint-inclusive edge arrays are the intended compatibility contract with
...
- `python -m skelhub graphgen -i test_data/lsys_gt/iter_4_8_step_1/Lnet_i4_0_tort_centreline_26conn.nii.gz -o /tmp/skelhub_graphgen_endpoints.graphml --verbose`, followed by an endpoint, world-coordinate, count, and 26-connectivity audit (`4 nodes`, `3 edges`).
...
- Added `centerline_voxel_points` containing exact float-cast source skeleton
  voxels.
- Kept `centerline_voxels` and `num_centerline_voxels` unchanged; the integer
...
- Added local ignored regression coverage in
  `tests/test_graphgen_centerline_coordinates.py`; `.gitignore` was not
  changed.
- Modified `README.md`, `docs/postprocessing.md`,
...
- Serialized voxel samples directly from `ProtoGraphEdge.voxels` as floats to
  avoid unnecessary inverse-affine round-trip error.
